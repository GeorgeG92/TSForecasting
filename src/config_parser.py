...
def config_parser(args):
	...
	with open(os.path.join(args.configpath, args.configname)) as file:
		...
		assert all(assertBetween(element, -0.0001, 1) for element in config['Forecasting']['LSTM']['GridSearchCV']['L2']), "L2 has to be between 0 and 1"
		assert all(isinstance(item, int)  for item in config['Forecasting']['LSTM']['GridSearchCV']['batch_size']), "BatchSize parameter has to an integer number"
		...
		assert all(assertBetween(element,  -0.0001, 1) for element in config['Forecasting']['LSTM']['GridSearchCV']['dropout']), "Dropout has to be between 0 and 1"
		assert all(isinstance(item, float) for item in config['Forecasting']['LSTM']['GridSearchCV']['learning_rate']), "LearningRate parameter has to be a float number"
		assert all(assertBetween(element,  -0.0001, 1) for element in config['Forecasting']['LSTM']['GridSearchCV']['learning_rate']), "LearningRate has to be between 0 and 1"
		...
		with open(os.path.join(args.configpath, args.configoptname)) as file:
			...
			if args.method=='LSTM':
				if not 'Optimal LSTM Parameters' in configOpt:
					...
					args.gridsearch = True
				else:
					if not all (k in configOpt['Optimal LSTM Parameters'] for k in ("L2", "batch_size", 'dropout', 'epochs', 'learning_rate', 'optimizer')): 
						logger.warning("Mode is set to use optimal parameters for LSTM training but {conf} is malformed".format(conf=args.configoptname))
						args.gridsearch = True
					else:
